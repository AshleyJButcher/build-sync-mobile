...
import { useCreateCostChange } from '../hooks/useProjectData';
...
interface AddCostChangeModalProps {
  visible: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export function AddCostChangeModal({
  visible,
  onClose,
  onSuccess,
}: AddCostChangeModalProps) {
  const theme = useTheme<Theme>();
  const { selectedProjectId } = useProjectStore();
  const createCostChange = useCreateCostChange();
  ...
  const handleSubmit = async () => {
    if (!selectedProjectId) {
      setErrors({ submit: 'No project selected. Please select a project first.' });
      return;
    }
    if (!validateForm()) {
      return;
    ...
    try {
      await createCostChange.mutateAsync({
      ...
      console.error('[AddCostChangeModal] Error creating cost change:', error);
      setErrors({
        submit: error instanceof Error ? error.message : 'Failed to create cost change',
      });
    }
  };
  ...
  const originalNum = Number(formData.original_cost);
  const newNum = Number(formData.new_cost);
  const difference = formData.original_cost && formData.new_cost &&
    !isNaN(originalNum) && !isNaN(newNum)
    ? newNum - originalNum
    : 0;
            ...
            >
              ...
              <TouchableOpacity
                style={[
                  ...
                  createCostChange.isPending && styles.disabledButton,
                ]}
                onPress={handleSubmit}
                disabled={createCostChange.isPending}
              >
                {createCostChange.isPending ? (
